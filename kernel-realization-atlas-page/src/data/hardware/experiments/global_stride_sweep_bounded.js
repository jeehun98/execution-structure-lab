const globalStrideSweepBoundedNoWrap = {
  ...
  summary:
    "주소를 wrap-around 하지 않는 조건에서 stride를 키워, actual work와 usable access envelope가 어떻게 빠르게 붕괴하는지 관찰합니다.",

  question:
    "주소를 배열 범위 밖으로 넘기지 않는 조건에서 stride가 커질 때, GPU의 실행 시간 곡선은 address dispersion보다 actual work collapse의 영향을 얼마나 강하게 받는가?",

  whyItMatters:
    "이 실험의 목적은 pure stride penalty를 측정하는 데 있지 않습니다. 오히려 no-wrap 조건에서 stride 증가가 실제 유효 access 수, total bytes actual, active thread 수를 얼마나 빠르게 줄이는지 관찰함으로써, wrapped 결과에서 보이는 시간 회복이 reuse 때문인지 usable work 감소 때문인지 분리해서 읽게 해주는 데 있습니다.",

  method: [
    "stride를 1에서 1024까지 증가시키되, 주소가 배열 범위를 벗어나면 더 이상 유효 access를 수행하지 않도록 구성합니다.",
    "requested_total_accesses와 accesses_per_thread는 동일하게 유지하지만, actual_total_accesses는 stride에 따라 달라집니다.",
    "stride별로 avg_ms, actual_total_accesses, total_bytes_actual, active_threads, warp_address_span_bytes를 함께 기록합니다.",
    "결과 곡선은 memory efficiency curve가 아니라 bounded execution envelope가 어떻게 무너지는지 보여주는 곡선으로 읽습니다.",
  ],
...
output[tid] = acc;`,
  ...
  observe: [
    ...
    "시간 감소가 address efficiency가 아니라 workload collapse를 얼마나 강하게 반영하는가",
    "큰 stride에서 active_threads 감소가 언제부터 나타나는가",
    "bounded no-wrap 조건에서 usable work surface가 어떻게 사라지는가",
  ],
  ...
  charts: [
    {
      ...
      yKeys: ["avg_ms"],
      summary:
        "시간은 전반적으로 감소하지만, 이 곡선은 efficiency gain보다 bounded work collapse의 영향을 더 강하게 반영합니다.",
    },
    {
      ...
      summary:
        "Stride 증가와 함께 실제 유효 access 수와 actual bytes가 얼마나 빠르게 줄어드는지 봅니다.",
    },
    {
      ...
      summary:
        "큰 stride에서 active thread 수까지 줄어들며, 같은 시간 곡선이 전혀 다른 실행 조건 위에 놓여 있음을 봅니다.",
    },
  ],
  ...
  interpretation: [
    ...
    "따라서 실행 시간 하락은 memory transaction 효율 향상보다는 workload collapse의 영향을 더 강하게 반영합니다.",
