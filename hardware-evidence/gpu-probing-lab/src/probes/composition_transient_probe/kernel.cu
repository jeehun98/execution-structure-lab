...
int shared_chain_count_for_condition(int condition_id) {
...
__device__ __forceinline__
int light_ready_count_for_condition(int condition_id, int ready_warp_count) {
    if (condition_id <= 4) {
        return ready_warp_count - shared_chain_count_for_condition(condition_id);
    }
...
__device__ __forceinline__
int global_stalled_count_for_condition(int condition_id, int warps_per_block) {
    if (condition_id <= 4) {
        return 4;
    }
...
int resolve_role_id(
    ...
    const int shared_chain_count =
        shared_chain_count_for_condition(condition_id);

    // ready 영역: warp 0~3
    // 그 안에서 앞쪽부터 shared_chain, 나머지는 light_alu.
...
__global__
void composition_transient_kernel(
    ...
    const int light_ready_count =
        light_ready_count_for_condition(condition_id, ready_warp_count);

    const int global_stalled_count =
        global_stalled_count_for_condition(condition_id, warps_per_block);
    ...
    if (lane_id == 0) {
        ...
        out_shared_chain_ready_count[global_warp_index] = shared_chain_count;
        out_light_ready_count[global_warp_index] = light_ready_count;
        out_global_stalled_count[global_warp_index] = global_stalled_count;
    }
...
__global__
void initialize_composition_transient_global_buffer(
    std::uint32_t* global_buffer,
    int global_buffer_size,
    std::uint32_t seed
) {
...
extern "C"
cudaError_t launch_initialize_composition_transient_global_buffer(
    std::uint32_t* global_buffer,
    int global_buffer_size,
    std::uint32_t seed
) {
    ...
    initialize_composition_transient_global_buffer<<<blocks, threads>>>(
        global_buffer,
        global_buffer_size,
        seed
    );
...
extern "C"
cudaError_t launch_composition_transient_kernel(
    std::uint64_t* out_progress,
    int* out_role_id,
    int* out_shared_chain_ready_count,
    int* out_light_ready_count,
    ...
    >>>(
        out_progress,
        out_role_id,
        out_shared_chain_ready_count,
        out_light_ready_count,
        out_global_stalled_count,
        global_buffer,
        global_buffer_size,
        cycle_budget,
        warps_per_block,
