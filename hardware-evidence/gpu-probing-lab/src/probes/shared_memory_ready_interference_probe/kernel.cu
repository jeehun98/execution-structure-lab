...
enum SharedReadyInterferenceRole {
    ROLE_LIGHT_ALU_READY = 0,
    ROLE_SHARED_NO_CONFLICT_READY = 1,
    ROLE_SHARED_BANK_CONFLICT_READY = 2,
    ROLE_SHARED_DEPENDENT_CHAIN_READY = 3,
    ROLE_DEPENDENT_GLOBAL_STALLED = 4
};
...
int resolve_role_id(
    ...
    // condition 5: all shared dependent chain
    if (condition_id == 5) {
    ...
    if (condition_id == 1) {
        return ROLE_SHARED_NO_CONFLICT_READY;
    }

    if (condition_id == 2) {
        return ROLE_SHARED_BANK_CONFLICT_READY;
    }
    ...
    // condition 4: mixed shared ready sources
...
__device__ __forceinline__
std::uint32_t run_shared_no_conflict(
    std::uint32_t x,
    const std::uint32_t* shared_data,
    int lane_id
) {
    // Lane별로 서로 다른 bank에 접근하도록 연속 index 사용.
    const int idx0 = lane_id;
    const int idx1 = 32 + lane_id;
    const int idx2 = 64 + lane_id;
...
__device__ __forceinline__
std::uint32_t run_shared_bank_conflict(
    std::uint32_t x,
    const std::uint32_t* shared_data,
    int lane_id
) {
    // 32-bit shared memory bank 기준으로 stride 32는 같은 bank 접근을 유도.
    // 모든 lane이 서로 다른 address지만 같은 bank group으로 몰리는 형태.
    const int idx0 = (lane_id * 32) & 1023;
    const int idx1 = ((lane_id * 32) + 1) & 1023;
    const int idx2 = ((lane_id * 32) + 2) & 1023;
...
std::uint32_t run_shared_dependent_chain(
    ...
    // shared memory load 결과가 다음 shared address를 결정.
...
__global__
void shared_memory_ready_interference_kernel(
    ...
    while ((clock64() - start) < cycle_budget) {
        ...
        else if (role == ROLE_SHARED_NO_CONFLICT_READY) {
            x = run_shared_no_conflict(x, shared_data, lane_id);
        }
        else if (role == ROLE_SHARED_BANK_CONFLICT_READY) {
            x = run_shared_bank_conflict(x, shared_data, lane_id);
        }
        else if (role == ROLE_SHARED_DEPENDENT_CHAIN_READY) {
            x = run_shared_dependent_chain(x, shared_data, lane_id);
        }
        else {
...
__global__
void initialize_shared_memory_ready_interference_global_buffer(
    std::uint32_t* global_buffer,
    int global_buffer_size,
    std::uint32_t seed
) {
...
extern "C"
cudaError_t launch_initialize_shared_memory_ready_interference_global_buffer(
    std::uint32_t* global_buffer,
    int global_buffer_size,
    std::uint32_t seed
) {
    ...
    initialize_shared_memory_ready_interference_global_buffer<<<
        blocks,
        threads
    >>>(
        global_buffer,
...
extern "C"
cudaError_t launch_shared_memory_ready_interference_kernel(
    std::uint64_t* out_progress,
    int* out_role_id,
    std::uint32_t* global_buffer,
    int global_buffer_size,
    ...
    shared_memory_ready_interference_kernel<<<
        blocks,
        threads_per_block,
        shared_bytes
    >>>(
