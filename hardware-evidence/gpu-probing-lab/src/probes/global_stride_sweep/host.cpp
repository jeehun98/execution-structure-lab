...
Result run_single_case(
  ...
  if (wrapped) {
    ...
    if (base_offset >= config.n) {
      ...
      r.actual_total_accesses =
          std::max<long long>(0, std::min<long long>(config.total_accesses, max_valid_logical));
    }

    r.active_threads =
        static_cast<int>(std::min<long long>(launched_threads, r.actual_total_accesses));
    r.total_bytes_actual =
        static_cast<long long>(r.actual_total_accesses) * sizeof(float);
  ...
  r.avg_ms = ms_sum / static_cast<float>(config.repeat);
  ...
  r.checksum = checksum;
  r.output_mean = sum / static_cast<double>(h_output.size());
...
std::string suite_to_json(const SuiteResult& suite) {
  ...
  oss << "    \"strides\": [";
  for (size_t i = 0; i < suite.config.strides.size(); ++i) {
    if (i) oss << ", ";
    oss << suite.config.strides[i];
  }
  oss << "],\n";
  ...
  oss << "    \"offset_values\": [";
  for (size_t i = 0; i < suite.config.offset_values.size(); ++i) {
    if (i) oss << ", ";
    oss << suite.config.offset_values[i];
  }
  oss << "],\n";

  oss << "    \"offset_representative_strides\": [";
  for (size_t i = 0; i < suite.config.offset_representative_strides.size(); ++i) {
    if (i) oss << ", ";
    oss << suite.config.offset_representative_strides[i];
  }
  oss << "],\n";
