...
__global__
void scheduler_phase_dummy_kernel(
    std::uint32_t* global_buffer,
    std::uint32_t* sink,
    int global_buffer_size,
    std::uint64_t cycle_budget,
    ...
    while ((clock64() - start) < cycle_budget) {
        const int idx = static_cast<int>((x + tid) & (global_buffer_size - 1));
        x ^= global_buffer[idx];
...
__global__
void initialize_scheduler_phase_global_buffer(
    std::uint32_t* global_buffer,
    int global_buffer_size,
    std::uint32_t seed
) {
...
extern "C"
cudaError_t launch_initialize_scheduler_phase_global_buffer(
    std::uint32_t* global_buffer,
    int global_buffer_size,
    std::uint32_t seed
) {
    ...
    initialize_scheduler_phase_global_buffer<<<blocks, threads>>>(
        global_buffer,
        global_buffer_size,
        seed
    );
...
extern "C"
cudaError_t launch_scheduler_phase_dummy_kernel(
    std::uint32_t* global_buffer,
    std::uint32_t* sink,
    int global_buffer_size,
    std::uint64_t cycle_budget,
    ...
    scheduler_phase_dummy_kernel<<<blocks, threads>>>(
        global_buffer,
        sink,
        global_buffer_size,
        cycle_budget,
...
extern "C"
cudaError_t launch_scheduler_phase_kernel(
    std::uint64_t* out_progress,
    int* out_role_id,
    std::uint32_t* global_buffer,
    int global_buffer_size,
