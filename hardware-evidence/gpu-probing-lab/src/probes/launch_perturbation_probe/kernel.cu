...
__global__
void launch_perturbation_measure_kernel(
    ...
    for (int i = tid; i < 1024; i += blockDim.x) {
        ...
        x ^= static_cast<std::uint32_t>(block_id * 0x9e3779b9u);
        x = hash_u32(x);
        shared_data[i] = x;
    }
    ...
    x ^= static_cast<std::uint32_t>(block_id * 0x45d9f3bu);
...
__global__
void launch_perturbation_dummy_kernel(
    std::uint32_t* global_buffer,
    std::uint32_t* sink,
    int global_buffer_size,
    std::uint64_t cycle_budget,
    ...
    std::uint32_t x =
        static_cast<std::uint32_t>(
            tid ^
            condition_id * 0x9e3779b9u ^
            batch_id * 0x85ebca6bu ^
            run_id * 0xc2b2ae35u
        );
    ...
    while ((clock64() - start) < cycle_budget) {
        if (dummy_mode == DUMMY_LIGHT_ALU) {
            x = hash_u32(x + 0x9e3779b9u);
        }
        else if (dummy_mode == DUMMY_GLOBAL_READ) {
            const int idx =
                static_cast<int>((x + tid) & (global_buffer_size - 1));

            x ^= global_buffer[idx];
            x = hash_u32(x);
        }
        else if (dummy_mode == DUMMY_GLOBAL_WRITE) {
            const int idx =
                static_cast<int>((x + tid) & (global_buffer_size - 1));

            global_buffer[idx] = hash_u32(x ^ global_buffer[idx]);
            x = hash_u32(x + static_cast<std::uint32_t>(idx));
        }
    }
...
__global__
void initialize_launch_perturbation_global_buffer(
    std::uint32_t* global_buffer,
    int global_buffer_size,
    std::uint32_t seed
) {
...
extern "C"
cudaError_t launch_initialize_launch_perturbation_global_buffer(
    std::uint32_t* global_buffer,
    int global_buffer_size,
    std::uint32_t seed
) {
    ...
    initialize_launch_perturbation_global_buffer<<<blocks, threads>>>(
        global_buffer,
        global_buffer_size,
        seed
    );
...
extern "C"
cudaError_t launch_launch_perturbation_dummy_kernel(
    std::uint32_t* global_buffer,
    std::uint32_t* sink,
    int global_buffer_size,
    std::uint64_t cycle_budget,
    ...
    launch_perturbation_dummy_kernel<<<dummy_blocks, dummy_threads>>>(
        global_buffer,
        sink,
        global_buffer_size,
        cycle_budget,
...
extern "C"
cudaError_t launch_launch_perturbation_measure_kernel(
    std::uint64_t* out_progress,
    int* out_role_id,
    std::uint32_t* global_buffer,
    int global_buffer_size,
    ...
    launch_perturbation_measure_kernel<<<
        blocks,
        threads_per_block,
        shared_bytes
    >>>(
