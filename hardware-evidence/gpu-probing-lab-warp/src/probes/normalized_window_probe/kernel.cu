...
__global__
void normalized_window_dummy_kernel(
    std::uint32_t* global_buffer,
    std::uint32_t* sink,
    int global_buffer_size,
    std::uint64_t cycle_budget,
    ...
    while ((clock64() - start) < cycle_budget) {
        ...
        x ^= global_buffer[idx];
        x = hash_u32(x + static_cast<std::uint32_t>(stride));
    }
...
__global__
void initialize_normalized_window_global_buffer(
    std::uint32_t* global_buffer,
    int global_buffer_size,
    std::uint32_t seed
) {
...
extern "C"
cudaError_t launch_initialize_normalized_window_global_buffer(
    std::uint32_t* global_buffer,
    int global_buffer_size,
    std::uint32_t seed
) {
    ...
    initialize_normalized_window_global_buffer<<<blocks, threads>>>(
        global_buffer,
        global_buffer_size,
        seed
    );
...
extern "C"
cudaError_t launch_normalized_window_dummy_kernel(
    std::uint32_t* global_buffer,
    std::uint32_t* sink,
    int global_buffer_size,
    std::uint64_t cycle_budget,
    ...
    normalized_window_dummy_kernel<<<blocks, threads>>>(
        global_buffer,
        sink,
        global_buffer_size,
        cycle_budget,
...
extern "C"
cudaError_t launch_normalized_window_kernel(
    std::uint64_t* out_progress,
    int* out_role_id,
    std::uint32_t* global_buffer,
    int global_buffer_size,
