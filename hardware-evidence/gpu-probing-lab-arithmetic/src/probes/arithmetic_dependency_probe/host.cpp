...
std::string role_name_from_id(int role_id) {
    switch (role_id) {
        case 0: return "independent_fma";
        case 1: return "dependent_fma_chain";
        case 2: return "independent_add";
        case 3: return "dependent_add_chain";
...
std::vector<ArithmeticProbeRecord> run_arithmetic_dependency_probe(
    ...
    for (int run = 0; run < repeat; ++run) {
        ...
        launch_arithmetic_dependency_kernel(
            d_records,
            blocks,
            threads_per_block,
            clock_budget_cycles,
